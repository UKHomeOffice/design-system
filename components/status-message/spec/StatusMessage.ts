...
describe("StatusMessage", () => {
	describe("when given a status", () => {
		...
		it("should show the status", () => {
			...
			expect(component.find(".hods-status-message__status").text()).toMatch(
				"My heading"
			);
		});
		...
		describe("and extra contents", () => {
			const component = mount(
				h(StatusMessage, { status: "My heading" }, "extra info")
			);
			it("should show the status", () => expect(component.text()).toMatch(/^My heading/));
			it("should show extra content", () => {
				expect(component.find(".hods-status-message__extra").text()).toMatch(
					"extra info"
				);
			});
			describe("with an action", () => {
				...
				it("should also show the action", () =>
					expect(
						component.find(".hods-status-message__actions li")
					).toHaveLength(1));
				it("should render the action", () =>
					expect(
						component.find(".hods-status-message__actions").text()
					).toEqual("Run"));
			});
			describe("with multiple actions", () => {
				...
				const component = mount(
					h(StatusMessage, { status: "My heading", actions }, "extra info")
				);
				it("should show the status", () =>
					expect(component.find(".hods-status-message__status").text()).toMatch(
						"My heading"

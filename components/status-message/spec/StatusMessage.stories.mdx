...
For an internal service, this was used at the top of the screen of a case management system to:

- show who owns the case
- show whether the case is read only
- allow users to take over the case

<Preview>
  <Story name="ExampleFromIntenalService">
    <StatusMessage status="Read only" actions={[
      { href: '#', text: 'Take over' }
    ]}>
      - Liam Davies is working on these results<br />

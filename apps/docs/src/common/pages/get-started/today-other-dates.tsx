...
`router.get(\'*\', function (req, res, next) {
  ...
  next()
      ...
      <pre><code>{`{{ date({'day': 'numeric', 'month': 'long', 'year': 'numeric'}, {'day': -1}) }}`}</code></pre>

      <h3>Current year</h3>
      <pre><code>{`{{ date({'year': 'numeric'}) }}`}</code></pre>

      <h3>Today's date short format</h3>
      <pre><code>{`{{ date({'day': 'numeric', 'month': 'numeric', 'year': 'numeric'}) }}`}</code></pre>

      <h3>Today's date only</h3>
      <pre><code>{`{{ today().day }}`}</code></pre>
